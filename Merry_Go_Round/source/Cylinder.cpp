...
: Shape(points * 2 * 2 + 2, 4 * points) {
	...
	for (int i = 0; i < points; i++) {
		...
		uv_buffer_data[2 * points + 2 * i] = 0.5 + 0.5*(radius/(height + radius)) * cos(M_PI * 2 * i / points);
		uv_buffer_data[2 * points + 2 * i + 1] = 0.5 + 0.5*(radius/(height + radius)) * sin(M_PI * 2 * i / points);
		...
		uv_buffer_data[2 * 2 * points + 2 * i] = 0.5 + (0.5) * cos(M_PI * 2 * i / points);
		uv_buffer_data[2 * 2 * points + 2 * i + 1] = 0.5 + (0.5) * sin(M_PI * 2 * i / points);
		...
		uv_buffer_data[2 * 2 * points + 2 * points + 2 * i] =
				0.5 + 0.5 * (radius / (height + radius)) * cos(M_PI * 2 * i / points);
		uv_buffer_data[2 * 2 * points + 2 * points + 2 * i + 1] =
				0.5 + 0.5 * (radius / (height + radius)) * sin(M_PI * 2 * i / points);
