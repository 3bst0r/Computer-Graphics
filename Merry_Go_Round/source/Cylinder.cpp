...
: Shape(points * 2 * 2 + 2, 4 * points) {
	...
	for (int i = 0; i < points; i++) {
		...
		normal_buffer_data[2 * i] = 0.5 * cos(M_PI * 2 * i / points);
		normal_buffer_data[2 * i + 1] = 0.5 * sin(M_PI * 2 * i / points);
		...
		normal_buffer_data[2 * points + 2 * i] = 0.5 * cos(M_PI * 2 * i / points); // ?
		normal_buffer_data[2 * points + 2 * i + 1] = 0.5 * sin(M_PI * 2 * i / points);
	...
	normal_buffer_data[4 * 2 * points] = 0.5;
	normal_buffer_data[4 * 2 * points + 1] = 0.5;
