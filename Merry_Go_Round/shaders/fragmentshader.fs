...
void main()
{
	...
	FragColor += kD * diff * (0.3 * vec4(lightColor1, 1.) + 0.7 * texture2D(myTextureSampler,UVcoords));
	...
	FragColor += kD * diff * (0.3 * vec4(lightColor2, 1.) + 0.7 * texture2D(myTextureSampler,UVcoords));
